...
class App {
    ...
    _loadMap(position) {
        ...
        const coords = [latitude, longitude];
        this._map = L.map('map').setView(coords, this._mapZoomllevel);//second parameter is for zoom of map when it first appears
    ...
    _moveToPopup(e) {
        ...
        this._map.setView(workout.coords, this._mapZoomllevel, {//In 3rd argument we can pass object of options
            animate: true,
            pan: {
                duration: 1,
            }
...
*/
    ...
    _getLocalStorage() {
        ...
        // console.log(data);
        if (!data) return;
        this._workouts = data;//in starting workouts array is empty but now it takes data from local storage
